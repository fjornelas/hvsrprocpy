...
def _plt_hvsr(freq, hvsr_mat, idx_select, hvsr_mean, hvsr_sd, hvsr_sd1,
              ...
              robust_est=False, sjb_avg=True, distribution='normal', mean_flag=False):
    """
        ...
        if option.lower() == 'q':
            ...
            if remove_index in idx_select:
                ...
                if sjb_avg:
                    ...
                    if mean_flag:
                        h1_mat_mean = np.mean(np.abs(h1_mat[idx_select]), axis=0)
                        h2_mat_mean = np.mean(np.abs(h2_mat[idx_select]), axis=0)
                        v_mat_mean = np.mean(np.abs(v_mat[idx_select]), axis=0)
                    else:
                    ...
                    if distribution == 'normal':
                        if mean_flag:
                            hvsr_mean = np.mean(hvsr_mat[idx_select], axis=0)
                            hvsr_sd = np.std(hvsr_mat[idx_select], axis=0)
                        else:
                            hvsr_mean = np.median(hvsr_mat[idx_select], axis=0)
                            hvsr_sd = np.std(hvsr_mat[idx_select], axis=0)
                        ...
                        if mean_flag:
                            hvsr_mat[hvsr_mat <= 0] = 1e-5
                            hvsr_mean = np.exp(np.mean(np.log(hvsr_mat[idx_select]), axis=0))
                            hvsr_sd = np.std(np.log(hvsr_mat[idx_select]), axis=0)
                        else:
                            hvsr_mat[hvsr_mat <= 0] = 1e-5
...
def _hvsr_plt_select(hvsr_list, robust_est=False, freq_hv_mean=None,
                     distribution='normal', plot_hvsr=True, sjb_avg=True, mean_flag=False):
    """
    ...
    if sjb_avg:
        ...
        if distribution == 'normal':
            if mean_flag:
                hvsr_mean = np.mean(hvsr_mat, axis=0)
                hvsr_sd = np.std(hvsr_mat, axis=0)
            else:
                hvsr_mean = np.median(hvsr_mat, axis=0)
                hvsr_sd = np.std(hvsr_mat, axis=0)
            ...
            if mean_flag:
                hvsr_mat[hvsr_mat <= 0] = 1e-5
                hvsr_mean = np.exp(np.mean(np.log(hvsr_mat), axis=0))
                hvsr_sd = np.std(np.log(hvsr_mat), axis=0)
            else:
                hvsr_mat[hvsr_mat <= 0] = 1e-5
    ...
    if plot_hvsr:
        idx_select, hvsr_mean, hvsr_sd, hvsr_sd1, h1_mat, h2_mat, v_mat, h1_mat_mean, h2_mat_mean, v_mat_mean = \
            _plt_hvsr(freq_hv_mean, hvsr_mat, idx_select, hvsr_mean, hvsr_sd, hvsr_sd1, h1_mat, h2_mat, v_mat,
                      ...
                      distribution=distribution, mean_flag=mean_flag)
    else:
        pass
...
def hvsr(h1, h2, v, dt, time_ts, output_dir, **kwargs):
    ...
    **kwargs
        ...
        mean_flag: boolean
             Indicates whether to use the mean instead of the median to compute the mean HVSR or FAS curves.
        pre_filter_is_causal: boolean
        ...
        output_fas_sel_curves: boolean
            Indicates whether to output the selected fas curves for each component.
        output_removed_fas: boolean
            Indicates whether to output the removed fas curves for each component.
    ...
    defaults = {'is_noise': True, 'eqk_filepath': None, 'output_pf_flnm': 'Test_', 'distribution': 'normal',
                ...
                'pre_filter_flag': True, 'mean_flag': False,
                ...
                'output_fas_sel_curves': False, 'output_removed_fas': False,
    ...
    fd_select = _hvsr_plt_select(hvsr_list=hvsr_list, robust_est=updated_kwargs['robust_est'],
                                 ...
                                 mean_flag=updated_kwargs['mean_flag'])
    ...
    if len(iidx_select) == 0:
        ...
        if updated_kwargs['output_fas_sel_curves']:

            fas_sel_out = np.full((len(freq_hv_mean), len(iidx_select)*3 + 1), np.nan)
            fas_sel_out[:, 0] = freq_hv_mean

            selected_windows = []

            for i, idx in enumerate(iidx_select):
                h1_smooth = hvsr_list['h1_smooth'][idx]
                h2_smooth = hvsr_list['h2_smooth'][idx]
                v_smooth = hvsr_list['v_smooth'][idx]
                if h1_smooth is not None and h2_smooth is not None and v_smooth is not None:
                    fas_sel_out[:, 3 * i + 1] = h1_smooth
                    fas_sel_out[:, 3 * i + 2] = h2_smooth
                    fas_sel_out[:, 3 * i + 3] = v_smooth
                    selected_windows.append(idx + 1)

            colnames = ['freq_Hz']
            for window_idx in selected_windows:
                colnames.extend([f'fas_h1_{window_idx-1}', f'fas_h2_{window_idx-1}', f'fas_v_{window_idx-1}'])

            fas_sel_out = pd.DataFrame(np.round(fas_sel_out, 5), columns=colnames)

            outputflname_fas_sel = updated_kwargs['output_pf_flnm'] + 'fas_sel.csv'

            if updated_kwargs['output_freq_min'] is not None and updated_kwargs['output_freq_min'] > np.min(
                    fas_sel_out.iloc[:, 0]):
                idx_min = np.where(fas_sel_out.iloc[:, 0] >= updated_kwargs['output_freq_min'])[0]
                fas_sel_out = fas_sel_out.iloc[idx_min]

            if updated_kwargs['output_freq_max'] is not None and updated_kwargs['output_freq_max'] < np.max(
                    fas_sel_out.iloc[:, 0]):
                idx_max = np.where(fas_sel_out.iloc[:, 0] <= updated_kwargs['output_freq_max'])[0]
                fas_sel_out = fas_sel_out.iloc[idx_max]

            fas_sel_out.to_csv(output_dir + '/' + outputflname_fas_sel, index=False)
        ...
        if updated_kwargs['output_removed_fas']:
            if len(idx_select) > len(iidx_select):
                idx_remove = [i for i in range(len(idx_select)) if idx_select[i] not in iidx_select]

                outputflname_fas_unsel = updated_kwargs['output_pf_flnm'] + 'fas_unsel.csv'
                fas_unsel_out = np.full((len(freq_hv_mean), len(idx_remove)*3 + 1), np.nan)
                fas_unsel_out[:, 0] = freq_hv_mean
                ...
                for i, idx in enumerate(idx_remove):
                    ...
                    v_smooth = hvsr_list['v_smooth'][idx]
                    if h1_smooth is not None and h2_smooth is not None and v_smooth is not None:
                        fas_unsel_out[:, 3 * i + 1] = h1_smooth
                        fas_unsel_out[:, 3 * i + 2] = h2_smooth
                        fas_unsel_out[:, 3 * i + 3] = v_smooth
                        removed_windows.append(idx_select[idx])

                colnames = ['freq_Hz']
                for window_idx in removed_windows:
                    colnames.extend([f'fas_h1_{window_idx}', f'fas_h2_{window_idx}', f'fas_v_{window_idx}'])

                fas_unsel_out = pd.DataFrame(np.round(fas_unsel_out, 5), columns=colnames)

                if updated_kwargs['output_freq_min'] is not None and updated_kwargs['output_freq_min'] > np.min(
                        fas_unsel_out.iloc[:, 0]):
                    idx_min = np.where(fas_unsel_out.iloc[:, 0] >= updated_kwargs['output_freq_min'])[0]
                    fas_unsel_out = fas_unsel_out.iloc[idx_min]

                if updated_kwargs['output_freq_max'] is not None and updated_kwargs['output_freq_max'] < np.max(
                        fas_unsel_out.iloc[:, 0]):
                    idx_max = np.where(fas_unsel_out.iloc[:, 0] <= updated_kwargs['output_freq_max'])[0]
                    fas_unsel_out = fas_unsel_out.iloc[idx_max]

                fas_unsel_out.to_csv(output_dir + '/' + outputflname_fas_unsel, index=False)
        ...
        if updated_kwargs['output_polar_curves']:
            ...
            tmp_hvsr_mat = np.empty((len(freq_polar), len(iidx_select)), dtype=np.float64)

            hvsr_data = hvsr_list['polar_hv_ratio']

            for i in range(len(polar_degs)):
                for j in range(len(iidx_select)):
                    tmp_hvsr_mat[:, j] = hvsr_data[:, j][i]
